...
void saveFrame(void)
{
	...
	KeyFrame[FrameIndex].dogPosHY = dogPosHY;
...
void interpolation(void)
{
	...
	KeyFrame[playIndex].incH = (KeyFrame[playIndex + 1].dogPosHY - KeyFrame[playIndex].dogPosHY) / i_max_steps;
...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		model = glm::rotate(model, glm::radians(dogPosHY), glm::vec3(0.0f, 0.0f, 1.0f));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
...
void Animation() {
	...
	if (play)
	{
		...
		else
		{
			...
			dogPosHY += KeyFrame[playIndex].incH;
